 ...
 #  print(ch)
   ...
   try :
     ...
     if get_video(call.from_user.id)!=None:
      ...
      bot.send_message (chat_id =int(call.data[6:]),text=get_heading(call.from_user.id) , reply_markup =confirmed_btn(call.from_user.id,post=True ))
...
def gju(m):
 if m.from_user.id==did:
    ...
    #print(tex)
     ...
     #print(tex)
    # print(ac)
     user=telebot.util.user_link(int(id))
     if ch==0:
      ch=[0]
...
def get_links(m):
   ...
   if get_video(m.from_user.id)!=None:
       print (s)
       bot.send_video(m.chat.id,video=get_video(m.from_user.id),caption=get_heading(m.from_user.id),reply_markup=btn(m.from_user.id,m.text))  
   elif get_photo(m.from_user.id)!=None:
     bot.send_photo(m.chat.id,photo=get_photo(m.from_user.id),caption=get_heading(m.from_user.id),reply_markup=btn(m.from_user.id,m.text))  
   else :
       bot.send_message(chat_id=m.from_user.id,text=get_heading(m.from_user.id),reply_markup=btn(m.from_user.id,m.text))
...
def get_channel(m):
 ...
 #if check_for_pre(m.from_user.id)==False:
...
# except :
# while p==False:
 # bot.reply_to (m, "Only Channels Are Allowed ") 
...
def start(m):
 ...
 #else :
#  bot.send_message(m.chat.id, text=s,reply_markup=free_1(),parse_mode='Html')
 #channel_btn(m.from_user.id ) 
...
#def gg(m):
  #  bot.reply_to(m,"Like Players ",reply_markup=likk())
...
# add_heading(m.from_user.id,bv)
 add_heading(m.from_user.id,bv)
 #print (m.forward_from_chat)
 bot.reply_to (m,text=bv,reply_markup=btn(m.from_user.id,m.text),disable_web_page_preview=True)
...
def vvv(m):
 ...
 bot.send_photo(m.chat.id,
                   photo=photo_id,
                   caption=bv,
                   reply_markup=btn(m.from_user.id,m.caption))
...
def query_text(query):
   ...
   # print (label)
    ...
    result = telebot.types.InlineQueryResultArticle(
        id='1',
        title="Button ✅",
        input_message_content=telebot.types.InputTextMessageContent(tg),
